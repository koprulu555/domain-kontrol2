...
jobs:
  check-and-update-domain:
    ...
    - name: Smart yeni domain kontrolü yap
      run: |
        # Mevcut domaini al veya varsayılanı kullan
        if [ -f "dizipaldomain.txt" ]; then
          CURRENT_DOMAIN=$(grep -o 'guncel_domain=.*' dizipaldomain.txt | cut -d'=' -f2)
          echo "Mevcut domain: $CURRENT_DOMAIN"
        else
          CURRENT_DOMAIN="https://dizipal1219.com/"
          echo "Yeni dizipaldomain.txt oluşturuluyor: $CURRENT_DOMAIN"
          echo "guncel_domain=$CURRENT_DOMAIN" > dizipaldomain.txt
        fi
        ...
        max_check = 2  # Maksimum kontrol sayısını 4'e çıkar
        latest_working_domain = None
        
        # Tüm domainleri kontrol etmek için for döngüsü
        for i in range(max_check):
            current_check_domain = f'https://dizipal{start_domain_number + i}.com/'
        ...
        if latest_working_domain:
            ...
            latest_domain_number = int(latest_working_domain.split('dizipal')[1].split('.com')[0])
            selected_domain = latest_working_domain
            
            # 1 domain daha üstünü kontrol et
            for extra in range(1, 2):
                next_domain = f'https://dizipal{latest_domain_number + extra}.com/'
            ...
            selected_domain = 'https://dizipal1219.com/'
            print(f'Hiç çalışan domain bulunamadı, varsayılan kullanılacak: {selected_domain}')
        
        # dizipaldomain.txt'yi güncelle (her zaman)
        with open('dizipaldomain.txt', 'w') as f:
